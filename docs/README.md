...
- **The one render component + its render protocol** — a single "subgraph-in → view-out" component, with the honesty-rendering conventions (the ◇-agent-glyph, the AI-wash tint, the confidence marks) pinned in it. **The two-axis read carries a first-encounter explain affordance:** the first time a viewer meets the **two distinct marks** (coherence/xscore vs validation — the latter often empty) or a `refusal`/`gap` outcome, the component surfaces a one-line inline explainer ("coherent ≠ validated — empty means not-yet-validated, not false"; "refusal/gap is the product being honest about its boundary, not an error"), dismissible + idempotent per viewer. This is the design-system answer to the least-technical persona's most likely confusion point ("why doesn't it just tell me if it's true?"); it is a render-convention, not a product screen.
